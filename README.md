# AstrBot 插件 · Warframe 查询助手（`astrbot_plugin_warframe`）

一个给 AstrBot 用的 Warframe 查询插件：**世界状态 / 市场查价 / 伤害计算与配卡识别 /
紫卡分析 / 遗物与部件反查 / 蹲点推送**，并支持渲染成 Orokin 风格图片卡片。

指令为**自由参数式**：`主指令 + 内容 + 附加项`，三者位置任意、空格分隔。
例如 `伤害 绝路 镀层3`、`遗物 出库 -2`、`仲裁 钢铁` 都可以。

> 本插件最初为 Warframe 国服氏族 **-Sida-JieKong-（四大皆空）** 自用而做，
> 现以 GPL-3.0 开源。数据与人格相关的私有集成**已移除**（见「开源版与自用版的差异」）。
...
| 品牌标识 | 通用插件名 + README 注明源自 SDJK 氏族 | 氏族品牌 |
